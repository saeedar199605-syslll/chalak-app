...
export default function RewardCalculationCenter({
  ...
  useEffect(() => {
    const fetchConfig = () => {
      const savedConfig = db.getMiscData<RewardConfig>('pe_reward_config', DEFAULT_CONFIG);
      if (savedConfig && savedConfig.multipliers && savedConfig.coefficients) {
  ...
  useEffect(() => {
    if (profiles.length > 0) {
      ...
      setConfig(prev => {
        ...
        if (changed) {
          const newCfg = { ...prev, coefficients: newCoefs };
          // Auto-save the new family to make integration seamless
          db.saveMiscData('pe_reward_config', newCfg);
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'محاسبات پاداش');
    XLSX.writeFile(workbook, `Rewards_${Date.now()}.xlsx`);
  };
                   ...
                   {config.coefficients.map((c, i) => (
                     <tr key={i} className="hover:bg-slate-800/30">
                       ...
                       {config.multipliers.map((m, j) => {
                         const val = evaluateFormula(config.formula || 'baseAmount * multiplier', { score: (m.maxScore+m.minScore)/2, baseAmount: c.baseAmount, multiplier: m.multiplier });
                         return (
