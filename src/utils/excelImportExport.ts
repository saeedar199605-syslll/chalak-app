...
export function downloadKasraExcelTemplate(employees: Employee[], period: string = 'نیمه اول ۱۴۰۵') {
  ...
  XLSX.utils.book_append_sheet(wb, ws, 'داده_های_حضور_غیاب_کسری');
  XLSX.writeFile(wb, `نمونه_اکسل_کسری_اصفهان_چالاک_${period.replace(/\s+/g, '_')}.xlsx`);
}
...
export function downloadMISExcelTemplate(employees: Employee[], period: string = 'نیمه اول ۱۴۰۵') {
  ...
  XLSX.utils.book_append_sheet(wb, ws, 'داده_های_تولید_MIS');
  XLSX.writeFile(wb, `نمونه_اکسل_MIS_تولید_اصفهان_چالاک_${period.replace(/\s+/g, '_')}.xlsx`);
}
...
export function downloadDynamicCriteriaExcelTemplate({
  ...
  XLSX.utils.book_append_sheet(wb, ws, 'ماتریس_شاخص_ها');
  XLSX.utils.book_append_sheet(wb, wsGuide, 'راهنمای_شاخص_ها');
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        reject(new Error('خطا در بارگذاری و تحلیل فایل اکسل: ' + err.message));
      }
    };

    reader.onerror = () => reject(new Error('خطا در دسترسی به فایل انتخابی'));
