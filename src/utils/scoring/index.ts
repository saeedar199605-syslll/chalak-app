...
export function calculateWeightedScore(scores: ScoreCalcItem[]): RecalculatedEvaluationResult {
  const scored = scores.filter(s => s.value > 0);
  if (!scored.length) {
    return {
      totalScore: 0,
      avg5: 0,
  ...
  const totalWeight = scored.reduce((acc, curr) => acc + curr.weight, 0);
  const weightedSum = scored.reduce((acc, curr) => acc + (curr.value * curr.weight), 0);
